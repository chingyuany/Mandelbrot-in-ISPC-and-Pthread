...
extern void mandelbrotSerial(
    ...
    int startRow, int numRows, int heightstart, int totalheight,
    int maxIterations,
    int output[]);
...
void *workerThreadStart(void *threadArgs)
{
    ...
    int divide = sqrt(args->numThreads);
    

    int range = args->width / divide;
    // TODO: Implement worker thread here.
    //int range = args->height / args->numThreads;
    int startRow = range * args->threadId;
  
    int heightstart = args->height / divide;
    int totalheight = args->height;
    if (args->threadId < divide)
    {
        totalheight = heightstart;
        heightstart = 0;
    }
    ...
    mandelbrotSerial(args->x0, args->y0, args->x1, args->y1, args->width, args->height, startRow, range, heightstart, totalheight, args->maxIterations, args->output);
